...
static void CreateDockSpace(std::vector<std::unique_ptr<Viewport>>& viewports)
{
  ...
  if (!initialized)
  {
    ...
    viewports.push_back(std::move(std::make_unique<SceneViewport>(SCENE_WINDOW_NAME)));

    ImGui::DockBuilderDockWindow(PROPERTIES_WINDOW_NAME.c_str(), properties);
    viewports.push_back(std::move(std::make_unique<PropertiesViewport>(PROPERTIES_WINDOW_NAME)));
